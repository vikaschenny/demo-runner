...
def spin_container(host):
    ...
    container_id = subprocess.check_output(['docker', 'run', '-d', '-it', 'container_image_name']).strip().decode()

    # Run playbook inside the container using Ansible runner
    ansible_command = f'docker exec {container_id} ansible-runner run /opt/logs.yaml'
